...
@Injectable()
export class CustomersTableConfigResolver implements Resolve<EntityTableConfig<CustomerInfo | Customer>> {

  constructor(private allEntitiesTableConfigService: AllEntitiesTableConfigService<CustomerInfo | Customer>,
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<EntityTableConfig<CustomerInfo | Customer>> {
    const groupParams = resolveGroupParams(route);
    const config = new EntityTableConfig<CustomerInfo | Customer>(groupParams);
  ...
  configDefaults(config: EntityTableConfig<CustomerInfo | Customer>) {
    config.entityType = EntityType.CUSTOMER;
    config.entityComponent = CustomerComponent;
    config.entityTabsComponent = GroupEntityTabsComponent<Customer>;
  ...
  configureEntityFunctions(config: EntityTableConfig<CustomerInfo | Customer>): void {
  ...
  configureCellActions(config: EntityTableConfig<CustomerInfo | Customer>, authState: AuthState):
  ...
  configureGroupActions(config: EntityTableConfig<CustomerInfo | Customer>): Array<GroupActionDescriptor<CustomerInfo>> {
    const actions: Array<GroupActionDescriptor<CustomerInfo>> = [];
    return actions;
  }

  configureAddActions(config: EntityTableConfig<CustomerInfo | Customer>): Array<HeaderActionDescriptor> {
    const actions: Array<HeaderActionDescriptor> = [];
    return actions;
  }

  private openCustomer($event: Event, customer: CustomerInfo, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
  ...
  manageCustomerUsers($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  manageCustomers($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  manageCustomerAssets($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  manageCustomerDevices($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  manageCustomerEntityViews($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  manageCustomerDashboards($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  manageCustomerEdges($event: Event, customer: Customer, config: EntityTableConfig<CustomerInfo | Customer>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  private navigateToChildCustomerPage(config: EntityTableConfig<CustomerInfo | Customer>,
                                      customer: Customer, page: string, forceReload = false) {
    this.router.navigateByUrl(`customers/all/${customer.id.id}${page}`, {onSameUrlNavigation: forceReload ? 'reload' : undefined});
  }

  onCustomerAction(action: EntityAction<Customer>, config: EntityTableConfig<CustomerInfo | Customer>): boolean {
