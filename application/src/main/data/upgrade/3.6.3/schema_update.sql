...
$$
    DECLARE
        ...
        insert_cursor CURSOR FOR SELECT json_data.key AS locale, json_data.value AS value
                                 FROM admin_settings a, json_each_text(((trim('"' FROM a.json_value::json ->> 'value'))::json ->> 'translationMap')::json) AS json_data
...
$$
    DECLARE
        insert_record  RECORD;
        insert_cursor CURSOR FOR SELECT a.entity_id AS tenant_id, json_data.key AS locale, json_data.value AS value
                                 FROM attribute_kv a, json_each_text((a.str_v::json ->> 'translationMap')::json) AS json_data
...
$$
    DECLARE
        tenantId uuid;
        insert_record  RECORD;
        insert_cursor CURSOR FOR SELECT a.entity_id AS customer_id, json_data.key AS locale, json_data.value AS value
                                 FROM attribute_kv a, json_each_text((a.str_v::json ->> 'translationMap')::json) AS json_data
