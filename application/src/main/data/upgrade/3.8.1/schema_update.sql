                ...
                LOOP
                    ...
                    IF (srSettings.str_v::jsonb -> 'pkgName' IS NOT NULL AND srSettings.str_v::jsonb->> 'pkgName' <> '') THEN
                        ...
                        IF androidApp IS NULL THEN
                            ...
                            VALUES (androidAppId, (extract(epoch from now()) * 1000), srSettings.entity_id, pkgName, 'ANDROID', 'DRAFT');
                            generatedBundleId := uuid_generate_v4();
                            ...
                            VALUES (generatedBundleId, (extract(epoch from now()) * 1000), srSettings.entity_id, 'Autogenerated for '|| pkgName, androidAppId, srSettings.settings,
                        ...
                        ELSE IF androidApp.tenant_id != '13814000-1dd2-11b2-8080-808080808080' THEN
                            UPDATE mobile_app_bundle SET self_registration_config = srSettings.entity_id,
                                                         terms_of_use = (SELECT settings FROM white_labeling WHERE type = 'TERMS_OF_USE' AND tenant_id = srSettings.tenant_id),
                                                         privacy_policy = (SELECT settings FROM white_labeling WHERE type = 'PRIVACY_POLICY' AND tenant_id = srSettings.tenant_id)
                        ...
                        IF iosApp IS NULL THEN
                            iosAppId := uuid_generate_v4();
                            INSERT INTO mobile_app(id, created_time, tenant_id, pkg_name, platform_type, status)
                            VALUES (iosAppId, (extract(epoch from now()) * 1000), srSettings.entity_id, pkgName, 'IOS', 'DRAFT');
                            IF generatedBundleId IS NULL THEN
                                generatedBundleId := uuid_generate_v4();
                                ...
                                VALUES (generatedBundleId, (extract(epoch from now()) * 1000), srSettings.entity_id, 'Autogenerated for '|| pkgName, iosAppId, srSettings.str_v,
                                        (SELECT settings FROM white_labeling WHERE type = 'TERMS_OF_USE' AND tenant_id = srSettings.entity_id),
                                        (SELECT settings FROM white_labeling WHERE type = 'PRIVACY_POLICY' AND tenant_id = srSettings.entity_id));
                            ELSE
                                UPDATE mobile_app_bundle SET ios_app_id = iosAppId WHERE id = generatedBundleId;
