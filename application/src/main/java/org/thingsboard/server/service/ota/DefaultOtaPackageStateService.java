...
public class DefaultOtaPackageStateService implements OtaPackageStateService {
    ...
    private void update(Device device, OtaPackageInfo otaPackage, long ts) {
        ...
        BasicTsKvEntry status = new BasicTsKvEntry(System.currentTimeMillis(), new StringDataEntry(getTelemetryKey(otaPackage.getType(), STATE), OtaPackageUpdateStatus.INITIATED.name()));

        telemetryService.saveAndNotify(tenantId, deviceId, Collections.singletonList(status), new FutureCallback<>() {
            @Override
            public void onSuccess(@Nullable Void tmp) {
                log.trace("[{}] Success save telemetry with target OtaPackage for device!", deviceId);
            }

            @Override
            public void onFailure(Throwable t) {
                log.error("[{}] Failed to save telemetry with target OtaPackage for device!", deviceId, t);
            }
        });

        List<AttributeKvEntry> attributes = new ArrayList<>();
        attributes.add(new BaseAttributeKvEntry(ts, new StringDataEntry(getAttributeKey(otaPackage.getType(), TITLE), otaPackage.getTitle())));
        attributes.add(new BaseAttributeKvEntry(ts, new StringDataEntry(getAttributeKey(otaPackage.getType(), VERSION), otaPackage.getVersion())));
        attributes.add(new BaseAttributeKvEntry(ts, new LongDataEntry(getAttributeKey(otaPackage.getType(), SIZE), otaPackage.getDataSize())));
        attributes.add(new BaseAttributeKvEntry(ts, new StringDataEntry(getAttributeKey(otaPackage.getType(), CHECKSUM_ALGORITHM), otaPackage.getChecksumAlgorithm().name())));
        attributes.add(new BaseAttributeKvEntry(ts, new StringDataEntry(getAttributeKey(otaPackage.getType(), CHECKSUM), otaPackage.getChecksum())));
    ...
    private void remove(Device device, OtaPackageType firmwareType) {
        telemetryService.deleteAndNotify(device.getTenantId(), device.getId(), DataConstants.SHARED_SCOPE, OtaPackageUtil.getAttributeKeys(firmwareType),
                new FutureCallback<>() {
                    @Override
                    public void onSuccess(@Nullable Void tmp) {
                        log.trace("[{}] Success remove target OtaPackage attributes!", device.getId());
                    ...
                    @Override
                    public void onFailure(Throwable t) {
                        log.error("[{}] Failed to remove target OtaPackage attributes!", device.getId(), t);
                    }
                });

        String idKey = OtaPackageUtil.getAttributeKey(firmwareType, ID);
