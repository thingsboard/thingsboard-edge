        ...
        switch (fromVersion) {
            ...
            case "2.4.1":
                ...
                try (Connection conn = DriverManager.getConnection(dbUrl, dbUserName, dbPassword)) {
                    ...
                    try {
                        conn.createStatement().execute("ALTER TABLE integration ADD COLUMN enabled boolean"); //NOSONAR, ignoring because method used to execute thingsboard database upgrade script
                    } catch (Exception e) {}
                }
                log.info("Schema updated.");
                break;
