 ...
 */
package org.thingsboard.server.service.integration.http.ffb;
...
@Slf4j
public class FfbHttpIntegration extends AbstractHttpIntegration<HttpIntegrationMsg> {
    ...
    public void init(TbIntegrationInitParams params) throws Exception {
        ...
        if (securityEnabled && json.has("headersFilter")) {
            ...
            for (Iterator<Entry<String, JsonNode>> it = headersFilterNode.fields(); it.hasNext(); ) {
                Entry<String, JsonNode> headerFilter = it.next();
    ...
    protected ResponseEntity doProcess(IntegrationContext context, HttpIntegrationMsg msg) throws Exception {
        if (checkSecurity(msg)) {
            ...
            if (result.isEmpty()) {
                ...
                JsonNode dataArray = msg.getMsg().get("data");
                if (dataArray.isArray()) {
                    String ts = dataArray.get(dataArray.size() - 1).get("time").asText();
                    HttpHeaders responseHeaders = getHttpHeaders(null, "application/json");
                    return new ResponseEntity("{\"result\":\"" + ts + "\"}", responseHeaders, HttpStatus.OK);
                } else {
                    return fromStatus(HttpStatus.NO_CONTENT);
                }
            }
    ...
    protected boolean checkSecurity(HttpIntegrationMsg msg) throws Exception {
        if (securityEnabled) {
            ...
            for (Entry<String, String> headerFilter : headersFilter.entrySet()) {
