...
public class DashboardEdgeTest extends AbstractEdgeTest {
    ...
    public void testSendDashboardToCloud() throws Exception {
        ...
        dashboardUpdateMsgBuilder.setConfiguration("{\"description\":\"\"}");
        ...
        AbstractMessage latestMessage = edgeImitator.getLatestMessage();
        Assert.assertTrue(latestMessage instanceof DashboardUpdateMsg);
        DashboardUpdateMsg latestDashboardUpdateMsg = (DashboardUpdateMsg) latestMessage;
        Assert.assertEquals(uuid.getMostSignificantBits(), latestDashboardUpdateMsg.getIdMSB());
        Assert.assertEquals(uuid.getLeastSignificantBits(), latestDashboardUpdateMsg.getIdLSB());

        UUID newDashboardId = new UUID(latestDashboardUpdateMsg.getIdMSB(), latestDashboardUpdateMsg.getIdLSB());

        Dashboard dashboard = doGet("/api/dashboard/" + newDashboardId, Dashboard.class);
