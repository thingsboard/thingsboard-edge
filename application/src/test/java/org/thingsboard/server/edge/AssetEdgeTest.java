...
public class AssetEdgeTest extends AbstractEdgeTest {
    ...
    public void testSendAssetToCloud() throws Exception {
        ...
        AbstractMessage latestMessage = edgeImitator.getLatestMessage();
        Assert.assertTrue(latestMessage instanceof AssetUpdateMsg);
        AssetUpdateMsg latestAssetUpdateMsg = (AssetUpdateMsg) latestMessage;
        Assert.assertEquals(uuid.getMostSignificantBits(), latestAssetUpdateMsg.getIdMSB());
        Assert.assertEquals(uuid.getLeastSignificantBits(), latestAssetUpdateMsg.getIdLSB());
