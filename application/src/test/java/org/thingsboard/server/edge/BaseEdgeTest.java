...
abstract public class BaseEdgeTest extends AbstractControllerTest {
    ...
    private Edge edge;
    private EntityGroup deviceEntityGroup;
    private Device initialDevice;
    private EntityGroup assetEntityGroup;
    ...
    private Device saveDevice(String deviceName, String type) throws Exception {
        ...
        device.setType(type);
        return doPost("/api/device", device, Device.class, "entityGroupId", deviceEntityGroup.getId().toString());
    }

    private Asset saveAsset(String assetName, String type) throws Exception {
        Asset asset = new Asset();
        asset.setName(assetName);
        asset.setType(type);
        return doPost("/api/asset", asset, Asset.class, "entityGroupId", assetEntityGroup.getId().toString());
    }
    ...
    private void testReceivedInitialData() throws Exception {
        ...
        Assert.assertEquals(4, entityGroupUpdateMsgList.size());

        for (EntityGroupUpdateMsg entityGroupUpdateMsg : entityGroupUpdateMsgList) {
            UUID entityGroupUUID = new UUID(entityGroupUpdateMsg.getIdMSB(), entityGroupUpdateMsg.getIdLSB());
            EntityGroup entityGroup = doGet("/api/entityGroup/" + entityGroupUUID.toString(), EntityGroup.class);
            Assert.assertNotNull(entityGroup);
            if (EntityType.DEVICE.name().equals(entityGroupUpdateMsg.getType())) {
                testDeviceEntityGroupRequestMsg(entityGroupUpdateMsg.getIdMSB(), entityGroupUpdateMsg.getIdLSB());
            }
            if (EntityType.ASSET.name().equals(entityGroupUpdateMsg.getType())) {
                testAssetEntityGroupRequestMsg(entityGroupUpdateMsg.getIdMSB(), entityGroupUpdateMsg.getIdLSB());
            }
    ...
    private void testDeviceEntityGroupRequestMsg(long msbId, long lsbId) throws Exception {
        ...
        Assert.assertEquals(initialDevice.getId(), receivedDeviceId);

        testAutoGeneratedCodeByProtobuf(deviceUpdateMsg);
    }

    private void testAssetEntityGroupRequestMsg(long msbId, long lsbId) throws Exception {
        EntityGroupRequestMsg.Builder assetEntitiesGroupRequestMsgBuilder = EntityGroupRequestMsg.newBuilder()
                .setEntityGroupIdMSB(msbId)
                .setEntityGroupIdLSB(lsbId)
                .setType(EntityType.ASSET.name());
        testAutoGeneratedCodeByProtobuf(assetEntitiesGroupRequestMsgBuilder);

        UplinkMsg.Builder uplinkMsgBuilder = UplinkMsg.newBuilder()
                .addEntityGroupEntitiesRequestMsg(assetEntitiesGroupRequestMsgBuilder.build());
        testAutoGeneratedCodeByProtobuf(uplinkMsgBuilder);
        ...
        Assert.assertEquals(initialAsset.getId(), receivedAssetId);
    ...
    private void testAssets() throws Exception {
        ...
        // 1
        edgeImitator.expectMessageAmount(1);
        Asset savedAsset = saveAsset("Edge Asset 2", "Building");
        doPost("/api/edge/" + edge.getId().getId().toString()
        ...
        // 4
        edgeImitator.expectMessageAmount(1);
        savedAsset = saveAsset("Edge Asset 3", "Building");
        doPost("/api/edge/" + edge.getId().getId().toString()
                + "/asset/" + savedAsset.getId().getId().toString(), Asset.class);
        Assert.assertTrue(edgeImitator.waitForMessages());
        latestMessage = edgeImitator.getLatestMessage();
    ...
    private void installation() throws Exception {
        ...
        deviceEntityGroup = new EntityGroup();
        ...
        initialDevice = saveDevice("Edge Device 1", CUSTOM_DEVICE_PROFILE_NAME);

        doPost("/api/edge/" + edge.getId().getId().toString()
                + "/entityGroup/" + deviceEntityGroup.getId().toString() + "/" + EntityType.DEVICE.name(), EntityGroup.class);

        assetEntityGroup = new EntityGroup();
        ...
        initialAsset = saveAsset("Edge Asset 1", "Building");
        doPost("/api/edge/" + edge.getId().getId().toString()
