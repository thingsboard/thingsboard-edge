...
@Data
public class ParticleBasicCredentials implements ParticleCredentials, ClientHttpRequestInterceptor {
    ...
    private void refreshSession(RestTemplate restTemplate, String baseUrl) {
        ...
        ResponseEntity<JsonNode> sessionInfo = restTemplate.postForEntity(baseUrl + "/oauth/token", loginRequest, JsonNode.class);
        if (sessionInfo.getStatusCode().equals(HttpStatus.OK)) {
            accessToken = sessionInfo.getBody().get("access_token").asText();
