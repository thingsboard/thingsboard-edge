...
public class JpaSecretDao extends JpaAbstractDao<SecretEntity, Secret> implements SecretDao {
    ...
    @Override
    public PageData<Secret> findByTenantId(UUID tenantId, PageLink pageLink) {
        return DaoUtil.toPageData(secretRepository.findByTenantId(tenantId, DaoUtil.toPageable(pageLink)));
    }

    @Override
    public Secret findByTenantIdAndExternalId(UUID tenantId, UUID externalId) {
        return DaoUtil.getData(secretRepository.findByTenantIdAndExternalId(tenantId, externalId));
    }

    @Override
    public SecretId getExternalIdByInternal(SecretId internalId) {
        return DaoUtil.toEntityId(secretRepository.getExternalIdById(internalId.getId()), SecretId::new);
    }
