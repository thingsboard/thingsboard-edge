...
@EqualsAndHashCode
@ToString
public final class SchedulerEventEntity implements SearchTextEntity<SchedulerEvent> {
    ...
    @Column(name = SCHEDULER_EVENT_CONFIGURATION_PROPERTY, codec = JsonCodec.class)
    ...
    public SchedulerEventEntity(SchedulerEvent schedulerEvent) {
        if (schedulerEvent.getId() != null) {
            this.id = schedulerEvent.getId().getId();
        }
    ...
    public SchedulerEvent toData() {
        ...
        schedulerEvent.setCreatedTime(UUIDs.unixTimestamp(id));
        if (tenantId != null) {
