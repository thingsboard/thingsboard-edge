...
public class CustomerServiceImpl extends AbstractEntityService implements CustomerService {
    ...
    private Customer saveCustomerInternal(Customer customer) {
        ...
        if (customer.getId() == null) {
            ...
            if (!customer.isPublic()) {
                ...
                EntityGroup users = entityGroupService.getOrCreateUserGroup(savedCustomer.getTenantId(), savedCustomer.getId(),
                        EntityGroup.GROUP_CUSTOMER_USERS_NAME, "Autogenerated Customer Users group with read-only permissions.");
                CustomerId roleCustomerId = null;
                if (customer.isSubCustomer()) {
                    roleCustomerId = customer.getParentCustomerId();
                }
                Role userRole = getOrCreateRole(savedCustomer.getTenantId(), roleCustomerId, RoleType.GENERIC,  Role.ROLE_CUSTOMER_USER_NAME,
                ...
                EntityGroup admins = entityGroupService.getOrCreateUserGroup(savedCustomer.getTenantId(), savedCustomer.getId(),
                        EntityGroup.GROUP_CUSTOMER_ADMINS_NAME, "Autogenerated Customer Administrators group with read-only permissions.");
                Role adminRole = getOrCreateRole(savedCustomer.getTenantId(), roleCustomerId, RoleType.GENERIC, Role.ROLE_CUSTOMER_ADMIN_NAME,
                ...
                EntityGroup publicUsers = entityGroupService.getOrCreateUserGroup(savedCustomer.getTenantId(), savedCustomer.getId(),
                        EntityGroup.GROUP_PUBLIC_USERS_NAME, "Autogenerated Public Users group with read-only permissions.");
                Role publicUserRole = getOrCreatePublicUserRole(savedCustomer);
                GroupPermission groupPermission = new GroupPermission();
                groupPermission.setRoleId(publicUserRole.getId());
                groupPermission.setTenantId(savedCustomer.getTenantId());
                groupPermission.setUserGroupId(publicUsers.getId());
                groupPermissionService.saveGroupPermission(savedCustomer.getTenantId(), groupPermission);
                getOrCreatePublicUsersEntityGroupRole(savedCustomer);
            }
        }
        return savedCustomer;
    }

    private Role getOrCreatePublicUsersEntityGroupRole(Customer customer) {
        return getOrCreateRole(customer.getTenantId(), customer.getId(), RoleType.GROUP, Role.ROLE_PUBLIC_USER_ENTITY_GROUP_NAME,
                GroupPermission.PUBLIC_USER_ENTITY_GROUP_PERMISSIONS, "Autogenerated Public User group role with read-only permissions.");
    }

    private Role getOrCreatePublicUserRole(Customer customer) {
        return getOrCreateRole(customer.getTenantId(), customer.getId(), RoleType.GENERIC, Role.ROLE_PUBLIC_USER_NAME,
                GroupPermission.PUBLIC_USER_PERMISSIONS, "Autogenerated Public User role with read-only permissions.");
    }

    private Role getOrCreateRole(TenantId tenantId, CustomerId customerId, RoleType type,
                                        String name, Object permissions, String description) {
        Optional<Role> roleOptional;
        if (customerId != null && !customerId.isNullUid()) {
            roleOptional = roleService.findRoleByByTenantIdAndCustomerIdAndName(tenantId, customerId, name);
        } else {
            roleOptional = roleService.findRoleByTenantIdAndName(tenantId, name);
        }
    ...
    public Customer findOrCreatePublicCustomer(TenantId tenantId, EntityId ownerId) {
        ...
        try {
            ...
            if (entityGroup.isPresent()) {
                ...
                List<Customer> result = customers.stream().filter(customer -> customer.getTitle().equals(PUBLIC_CUSTOMER_TITLE)).collect(Collectors.toList());
                if (result.isEmpty()) {
    ...
    public EntityGroup findOrCreatePublicUserGroup(TenantId tenantId, EntityId ownerId) {
        ...
        try {
            Optional<EntityGroup> publicUsersOptional = entityGroupService.findEntityGroupByTypeAndName(publicCustomer.getTenantId(), publicCustomer.getId(),
                    EntityType.USER, EntityGroup.GROUP_PUBLIC_USERS_NAME).get();
            if (publicUsersOptional.isPresent()) {
                return publicUsersOptional.get();
            } else {
                EntityGroup publicUsers = entityGroupService.getOrCreateUserGroup(publicCustomer.getTenantId(), publicCustomer.getId(),
                        EntityGroup.GROUP_PUBLIC_USERS_NAME, "Autogenerated Public Users group with read-only permissions.");
                Role publicUserRole = getOrCreatePublicUserRole(publicCustomer);
                GroupPermission groupPermission = new GroupPermission();
                groupPermission.setRoleId(publicUserRole.getId());
                groupPermission.setTenantId(publicCustomer.getTenantId());
                groupPermission.setUserGroupId(publicUsers.getId());
                groupPermissionService.saveGroupPermission(publicCustomer.getTenantId(), groupPermission);
                return publicUsers;
            }
        } catch (InterruptedException | ExecutionException e) {
            throw new RuntimeException("Failed to find public users group!", e);
        }
    }
    ...
    public ListenableFuture<Optional<EntityGroup>> findPublicUserGroup(TenantId tenantId, CustomerId publicCustomerId) {
        ...
        return entityGroupService.findEntityGroupByTypeAndName(tenantId, publicCustomerId, EntityType.USER, EntityGroup.GROUP_PUBLIC_USERS_NAME);
    }
    ...
    public Role findOrCreatePublicUserEntityGroupRole(TenantId tenantId, EntityId ownerId) {
        ...
        Validator.validateEntityId(ownerId, INCORRECT_OWNER_ID + ownerId);
        Customer publicCustomer = findOrCreatePublicCustomer(tenantId, ownerId);
        Role publicUserRole = getOrCreatePublicUsersEntityGroupRole(publicCustomer);
