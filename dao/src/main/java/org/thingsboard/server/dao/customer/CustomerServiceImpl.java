...
public class CustomerServiceImpl extends AbstractEntityService implements CustomerService {
    ...
    public Customer saveCustomer(Customer customer) {
        ...
        if (customer.getId() == null) {
            ...
            if (!customer.isPublic()) {
                ...
                Role userRole = getOrCreateGenericRole(savedCustomer, Role.ROLE_CUSTOMER_USER_NAME,
                ...
                Role adminRole = getOrCreateGenericRole(savedCustomer, Role.ROLE_CUSTOMER_ADMIN_NAME,
                ...
                Role publicUserRole = getOrCreatePublicUsersRole(savedCustomer);
            }
        }
        return savedCustomer;
    }

    private Role getOrCreateGenericRole(Customer customer, String name, Map<Resource, List<Operation>> permissions, String description) {
    ...
    private Role getOrCreatePublicUsersRole(Customer customer) {
        Optional<Role> roleOptional;
        if (customer.isSubCustomer()) {
            roleOptional = roleService.findRoleByByTenantIdAndCustomerIdAndName(customer.getTenantId(), customer.getParentCustomerId(), Role.ROLE_PUBLIC_USER_NAME);
        } else {
            roleOptional = roleService.findRoleByTenantIdAndName(customer.getTenantId(), Role.ROLE_PUBLIC_USER_NAME);
        }
        if (!roleOptional.isPresent()) {
            Role role = new Role();
            role.setTenantId(customer.getTenantId());
            ...
            role.setName(Role.ROLE_PUBLIC_USER_NAME);
            role.setType(RoleType.GROUP);
            role.setPermissions(mapper.valueToTree(Arrays.asList(Operation.READ, Operation.RPC_CALL, Operation.READ_ATTRIBUTES, Operation.READ_TELEMETRY)));
            JsonNode additionalInfo = role.getAdditionalInfo();
            if (additionalInfo == null) {
                additionalInfo = mapper.createObjectNode();
            }
            ((ObjectNode)additionalInfo).put("description", "Autogenerated Public User role with read-only group permissions.");
            role.setAdditionalInfo(additionalInfo);
            return roleService.saveRole(customer.getTenantId(), role);
        } else {
            return roleOptional.get();
        ...
        try {
            ...
            if (entityGroup.isPresent()) {
                ...
                if (result.isEmpty()) {
                    ...
                    return saveCustomer(publicCustomer);
                } else {
                    return result.get(0);
                }
            } else {
    ...
    @Override
    public Role findOrCreatePublicUserRole(TenantId tenantId, EntityId ownerId) {
        log.trace("Executing findOrCreatePublicUserRole, tenantId [{}], ownerId [{}]", tenantId, ownerId);
        Validator.validateId(tenantId, INCORRECT_TENANT_ID + tenantId);
        Validator.validateEntityId(ownerId, INCORRECT_OWNER_ID + ownerId);
        Customer publicCustomer = findOrCreatePublicCustomer(tenantId, ownerId);
        Role publicUserRole = getOrCreatePublicUsersRole(publicCustomer);
