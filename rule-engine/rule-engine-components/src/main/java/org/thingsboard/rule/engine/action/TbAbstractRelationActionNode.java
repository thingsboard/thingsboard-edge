...
public abstract class TbAbstractRelationActionNode<C extends TbAbstractRelationActionNodeConfiguration> implements TbNode {
    ...
    protected ListenableFuture<EntityId> getTargetEntityId(TbContext ctx, TbMsg msg) {
        ...
        switch (entityType) {
                ...
                if (createEntityIfNotExists) {
                    return Futures.transform(customerByTitleFuture, customerOpt -> {
                        ...
                        synchronized (entitiesCreationLocks.computeIfAbsent(entityCreationLockKey, k -> new Object())) {
                            ...
                            var ownerId = ctx.getPeContext().getOwner(tenantId, msg.getOriginator());
            ...
            case CONVERTER -> {
                return ctx.getDbCallbackExecutor().executeAsync(() -> {
                    var converterService = ctx.getPeContext().getConverterService();
                    var converterOpt = converterService.findConverterByName(ctx.getTenantId(), targetEntityName);
                    if (converterOpt.isPresent()) {
                        return converterOpt.get().getId();
                    }
                    throw new NoSuchElementException("Data converter with name '" + targetEntityName + "' doesn't exist!");
                });
            }
            case ROLE -> {
                return ctx.getDbCallbackExecutor().executeAsync(() -> {
                    var roleService = ctx.getPeContext().getRoleService();
                    var roleOpt = roleService.findRoleByTenantIdAndName(ctx.getTenantId(), targetEntityName);
