...
public class TbAlarmsCountNodeTest {
    ...
    public void init() {
        ...
        config.setCalculateForChildEntities(true);
    ...
    private void performAlarmsCountTest(boolean genFailures) throws TbNodeException {
        ...
        verify(ctx, new Times(totalEntities*3)).tellNext(captor.capture(), eq(SUCCESS));
    ...
    private List<AlarmInfo> generateAlarms(EntityId entityId, List<AlarmInfo> childAlarms) {
        ...
        for (int i=0;i<alarmsCount;i++) {
            ...
            when(alarmService.findAlarmInfoByIdAsync(alarm.getId())).thenReturn(Futures.immediateFuture(alarm));
        }
