...
All paths are relative to this project (`tb-edge-ce`) root unless otherwise noted. Sibling projects must be cloned as directories next to this project.

**Expected directory layout:**
```
<parent-dir>/
├── tb-ce/                  # ThingsBoard Server CE
├── tb-edge-ce/             # This project (ThingsBoard Edge CE)
...
|---------|------|---------|
| ThingsBoard Server CE | `../tb-ce` | Cloud-side sync logic (CE) |
| ThingsBoard Edge CE | `.` (this project) | Edge-side sync logic (CE) |
...
- **Server diff (Calculated Fields):** `.claude/skills/edge-cloud-entities-sync/server-calculated-field-sync.diff`
- **Edge diff (Calculated Fields):** `.claude/skills/edge-cloud-entities-sync/edge-calculated-field-sync.diff`
- **Black Box Tests diff (Calculated Fields):** `.claude/skills/edge-cloud-entities-sync/bbt-calculated-field-sync.diff`
...
**File:** `../tb-ce/common/data/src/main/java/org/thingsboard/server/common/data/edge/EdgeEventType.java`
...
**File:** `../tb-ce/common/data/src/main/java/org/thingsboard/server/common/data/id/EntityIdFactory.java`
...
**File:** `../tb-ce/common/edge-api/src/main/proto/edge.proto`
...
```bash
cd ../tb-ce && mvn generate-sources -pl common/edge-api
```
...
`../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/rpc/processor/`
...
**File:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/EdgeMsgConstructorUtils.java`
...
**File:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/EdgeEventSourcingListener.java`
...
**File:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/EdgeContextComponent.java`
...
**File:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/rpc/processor/BaseEdgeProcessor.java`
...
**File:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/rpc/EdgeGrpcSession.java`
...
**File:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/rpc/EdgeSyncCursor.java`

**For Pattern A (standalone):** Create a new `EdgeEventFetcher` and add it to the fetcher list.

**Fetcher location:** `../tb-ce/application/src/main/java/org/thingsboard/server/service/edge/rpc/fetch/`
...
**File:** `../tb-ce/application/src/test/java/org/thingsboard/server/edge/NewEntityEdgeTest.java`
...
**File:** `../tb-ce/application/src/test/java/org/thingsboard/server/edge/imitator/EdgeImitator.java`

- Add handling for the new protobuf message type in the imitator's message collection

### Step 13b: Copy test class with @Ignore to Edge projects

The `NewEntityEdgeTest` from Step 12 is written in the Server project. When Server merges into Edge, this test will arrive in Edge — but it will fail because the Edge project doesn't have the server-side test infrastructure (EdgeImitator acts as edge, not cloud).

**To prevent build failures after merge**, copy the test file into both Edge CE and Edge PE **now**, with `@Ignore` on every `@Test` method:

**File (Edge CE):** `application/src/test/java/org/thingsboard/server/edge/NewEntityEdgeTest.java`
**File (Edge PE):** `../tb-edge-pe/application/src/test/java/org/thingsboard/server/edge/NewEntityEdgeTest.java`

- Copy the exact same test class from the Server project
- Add `@Ignore` annotation (from `org.junit.Ignore`) above every `@Test` method
- Use the appropriate license header (Apache 2.0 for CE, COMPANY CONFIDENTIAL for PE)
- When the merge eventually arrives, the `@Ignore` annotations will be conflict markers — resolve by keeping `@Ignore` until the tests are adapted for edge

```java
@Ignore
@Test
public void testNewEntity_create_update_delete_fromCloud() throws Exception {
    // ...
}
```
...
All Edge-specific changes go in this project (`.`).

### Step 14: Add CloudEventType enum entry + Extend EntityIdFactory (Edge project)

**File:** `common/data/src/main/java/org/thingsboard/server/common/data/cloud/CloudEventType.java`
...
**File:** `application/src/main/java/org/thingsboard/server/service/cloud/rpc/processor/NewEntityCloudProcessor.java`
...
**File:** `application/src/main/java/org/thingsboard/server/service/cloud/CloudEventSourcingListener.java`
...
**File:** `application/src/main/java/org/thingsboard/server/service/cloud/DefaultCloudNotificationService.java`
...
**File:** `application/src/main/java/org/thingsboard/server/service/cloud/event/UplinkMsgMapper.java`
...
**File:** `application/src/main/java/org/thingsboard/server/service/cloud/DefaultDownlinkMessageService.java`
...
**File:** `application/src/main/java/org/thingsboard/server/service/cloud/CloudContextComponent.java`
...
### Edge project (this project, `.`)

#### Common (both Pattern A and B)
- [ ] `NewEntityEdgeTest` with `@Ignore` — copy test class from Server project, add `@Ignore` on every `@Test` method to prevent build failure after Server→Edge merge (see Step 13b)
...
### Edge PE (`../tb-edge-pe`)
Same checklist as Edge CE (including `NewEntityEdgeTest` with `@Ignore`) — all items apply identically at PE paths, **plus:**
...
- **Server PE diff (Calculated Fields):** `.claude/skills/edge-cloud-entities-sync/server-calculated-field-sync-pe.diff`
- **Edge PE diff (Calculated Fields):** `.claude/skills/edge-cloud-entities-sync/edge-calculated-field-sync-pe.diff`
- **Black Box Tests PE diff (Calculated Fields):** `.claude/skills/edge-cloud-entities-sync/bbt-calculated-field-sync-pe.diff`

---

## Commit, Merge & Push

After all code changes are implemented and compiled across all 6 projects, follow these three phases to prepare for PR review.

### Phase 1: Commit all changes on feature branches

Create a feature branch (e.g., `edge-<entity>-sync`) in each project and commit:

1. **tb-ce** (`../tb-ce`) — all server-side changes
2. **tb-pe** (`../tb-pe`) — PE server-side changes
3. **tb-edge-ce** (this project) — edge-side changes including `@Ignore` test
4. **tb-edge-pe** (`../tb-edge-pe`) — PE edge-side changes including `@Ignore` test + RoleCloudProcessor
5. **edge-black-box-test** (`../edge-black-box-test`) — CE BBT
6. **edge-black-box-test-pe** (`../edge-black-box-test-pe`) — PE BBT

Use the same branch name across all 6 projects.

### Phase 2: Merge upstream → downstream (on feature branches)

Since code is implemented independently in each project, merges establish the upstream→downstream relationship. Without merging, future merges along these directions would create conflicts.

**Merge order (must be sequential — each merge depends on the previous):**

1. **tb-ce → tb-pe**: `cd ../tb-pe && git fetch <tb-ce-path> <branch> && git merge FETCH_HEAD`
2. **tb-ce → tb-edge-ce**: `git fetch <tb-ce-path> <branch> && git merge FETCH_HEAD`
3. **tb-pe → tb-edge-pe**: `cd ../tb-edge-pe && git fetch <tb-pe-path> <branch> && git merge FETCH_HEAD`
4. **tb-edge-ce → tb-edge-pe**: `cd ../tb-edge-pe && git fetch <tb-edge-ce-path> <branch> && git merge FETCH_HEAD`
5. **edge-black-box-test → edge-black-box-test-pe**: `cd ../edge-black-box-test-pe && git fetch <bbt-ce-path> <branch> && git merge FETCH_HEAD`

**Conflict resolution:** Since the same changes were made independently, conflicts are "both sides added identical code". Resolve by keeping the downstream (ours) version:
```bash
git checkout --ours <conflicted-files>
git add -A && git commit --no-edit
```

Unrelated conflicts (ui-ngx, msa/ files) may also appear due to divergence between repos — keep ours for those too. For `modify/delete` conflicts on files deleted in edge (e.g., `msa/js-executor/`), use `git rm` to remove them.

### Phase 3: Push to remotes

Push each project's feature branch to the appropriate remote:

| Project | Remote | Command |
|---------|--------|---------|
| tb-ce | `origin` (your fork) | `cd ../tb-ce && git push origin <branch>` |
| tb-pe | `origin` (your fork) | `cd ../tb-pe && git push origin <branch>` |
| tb-edge-ce | `upstream` (org repo) | `git push upstream <branch>` |
| tb-edge-pe | `upstream` (org repo) | `cd ../tb-edge-pe && git push upstream <branch>` |
| edge-black-box-test | `origin` (org repo) | `cd ../edge-black-box-test && git push origin <branch>` |
| edge-black-box-test-pe | `origin` (org repo) | `cd ../edge-black-box-test-pe && git push origin <branch>` |

After pushing, create PRs manually targeting the `rc` branch in each repo. For fork-based repos (tb-ce, tb-pe), create cross-fork PRs to the upstream org repo.
